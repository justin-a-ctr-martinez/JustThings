...
class GUI:
    ...
    def _browse_path(self, param: ActionParameter, var: tk.StringVar):
        ...
        if param.type == "path":
            if param.name.lower() in ["directory", "dest", "wc", "working_copy"]:
    ...
    def _on_wc_select(self, event):
        ...
        if selection:
            index = selection[0]
            wc_path = self.wc_listbox.get(index)
    ...
    def _refresh_working_copies(self):
        ...
        for wc in self.config_manager.config.working_copies:
            ...
            if display_name not in self.working_copies:
